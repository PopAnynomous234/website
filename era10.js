...
const games = [
    ...
    {
        title: "1",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS856EeuEIpQvzYp6DqeyA2hgghQjlrYWYQvg&s",
        url: "1.html"
    },
    {
        title: "9007199254740992",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQNfO6bcLqc1REl8cx6nvH-WEi8E_Z_XbBS1Q&s",
        url: "900.html"
    },
    {
        title: "Super Liquid Soccer",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR_r5XxD4-aM1Y8n7K2XOF0AWMW7ruxbKwNqQ&s",
        url: "liquidsoccer.html"
    },
    {
        title: "Curveball 3D",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSypB-8H11kRTwSy8tj_hGrsj1HMzIMCOQCMA&s",
        url: "curveball.html"
    },
    {
        title: "Super Santa Kicker",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQino6RAHBwpUEmm6efTkyufyUlJPvluHe40g&s",
        url: "santakick.html"
    },
    {
    ...
    {
        title: "Stick Fighter",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTqx3LAmfCYz50hs7Yx0-5CY4KKtwnVvuULqg&s",
        url: "stickfi.html"
    },
    {
        title: "Stickman Clash",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcStUj0tAKIWtxX3AFdptFR3Mmh3TV9zi1wg4Q&s",
        url: "stickmanclash.html"
    },
    {
        title: "FNAF SB",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQLp4bCSPjO2P9wW4Kz_2bhIlRJGrNGKdzLvg&s",
        url: "fnafsb.html"
    },
    {
        title: "Football Bros",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR1QZlCsuCHiEXkz2vcRrMtXvwLy4ybakJqsQ&s",
        url: "fottball.html"
    },
    {
        title: "Soccer Random",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTzf51eqTZ5dtKx3nNj4iFSk5mke5QmDQkjJA&s",
        url: "soccerbro.html"
    },
    {
        title: "Volley Random",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcROi6boMnZAX-pV_HieCObQNI0JbdBuT1zgAA&s",
        url: "volley.html"
    },
    {
        title: "Going Balls",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTqNQKhgnMp58PzqO9anLzb1wif8qtbwA5fpg&s",
        url: "goingball.html"
    }
...
/**
 * UI: Displays the Game of the Day banner
 */
function displayGameOfTheDay(game, views) {
    let banner = document.getElementById('gotd-banner');
    if (!banner) {
        banner = document.createElement('div');
        banner.id = 'gotd-banner';
        // Styled with your Neo-Mint (#26ff9a) and Glassmorphism aesthetic
        banner.style = `
            ...
            max-width: 800px;
            text-align: center;
            box-shadow: 0 8px 32px 0 rgba(0, 0, 0, 0.37), 0 0 15px rgba(38, 255, 154, 0.1);
    ...
    banner.innerHTML = `
        <h3 style="color: #26ff9a; text-transform: uppercase; letter-spacing: 2px; margin-bottom: 15px;">🔥 Game of the Day</h3>
        <div style="display: flex; align-items: center; justify-content: center; gap: 30px; flex-wrap: wrap;">
            <img src="${game.image}" style="width: 120px; height: 120px; object-fit: cover; border-radius: 15px; border: 2px solid #26ff9a;">
            <div style="text-align: left;">
                <h2 style="margin: 0; font-size: 1.8rem; color: #fff;">${game.title}</h2>
                <p style="color: rgba(255,255,255,0.7); margin: 5px 0;">Trending with <b>${views}</b> plays today!</p>
            </div>
        </div>
    `;
}
...
/**
 * Generates game cards and handles view logic
 */
async function generateCards() {
    const container = document.getElementById('card-container');
    container.innerHTML = '<div style="color: #26ff9a;">Scanning Database...</div>';
    ...
    try {
        ...
        // Fetch All-Time Views and Daily Views in parallel for efficiency
        const [totalSnapshot, dailySnapshot] = await Promise.all([
            get(ref(database, "gameViews")),
            get(ref(database, `dailyViews/${today}`))
        ]);

        const allViews = totalSnapshot.exists() ? totalSnapshot.val() : {};
        const dailyData = dailySnapshot.exists() ? dailySnapshot.val() : {};

        // Immediately disconnect to save Spark plan connections
        goOffline(database);

        // Logic to find Game of the Day
        let topGameId = null;
        let maxDailyViews = 0;

        for (const [id, count] of Object.entries(dailyData)) {
        ...
        const gotdMatch = games.find(g => g.url.replace('.html', '') === topGameId);
        if (gotdMatch && maxDailyViews > 0) {
            displayGameOfTheDay(gotdMatch, maxDailyViews);
        }
        ...
        games.forEach(game => {
            ...
            card.addEventListener('click', async () => {
                ...
                const totalRef = ref(database, "gameViews/" + gameId);
                const dailyRef = ref(database, `dailyViews/${today}/${gameId}`);

                // Atomically increment both counters
                await Promise.all([
                    runTransaction(totalRef, (curr) => (curr || 0) + 1),
                    runTransaction(dailyRef, (curr) => (curr || 0) + 1)
                ]);

                sessionStorage.setItem('currentGameUrl', JSDELIVR_BASE_URL + game.url);
                sessionStorage.setItem('currentGame', game.title);
        ...
        goOffline(database);
        container.innerHTML = 'Error loading games. Check console.';
    }
}
